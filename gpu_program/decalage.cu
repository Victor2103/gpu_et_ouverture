...
// code=decalage && nvcc -o $code.o $code.cu && ./$code.o
// code=decalage && nvcc -arch=sm_35 -o $code.o $code.cu && ./$code.o
...
__global__ void d_shift_oneeach(int* d_a, int* d_b, int size) {
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    if (index < size) {
        if (index==0)
        {
            d_b[index] = d_a[size -1];
        }
        else 
        {
...
void h_shift(int *tableau, int sizeTableau, int *tableauDecale)
{
    int i = 0;
...
void shift(int* a, int* b, int size) {
    ...
    if (*deviceCount == 0) {
        ...
        d_shift_oneeach<<<blocksPerGrid, threadsPerBlock>>>(d_a, d_b, size);
        gpuErrchk(cudaPeekAtLastError());
